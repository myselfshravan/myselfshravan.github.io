      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-4xl mx-auto">
          {portfolioData.work.map((job, index) => {
            const reportUrl = 'reportUrl' in job ? job.reportUrl : null;
            const reportLabel = 'reportLabel' in job ? job.reportLabel : 'Read the full write-up';
                  ...
                  <CardContent>
                    ...
                    {reportUrl && (
                      <a
                        href={reportUrl}
                        onClick={() => {
                          trackExternalLink(reportUrl, `Report: ${job.company}`);
                          posthog.capture('work_report_clicked', { company: job.company });
                        }}
                        className="mt-4 inline-flex items-center gap-2 text-sm font-mono text-primary hover:text-primary/80 transition-colors"
                      >
